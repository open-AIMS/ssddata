...
test_that("Timeframe, where present, is chronic or short_term", {
  items <- shipped_data_frames()
  items <- items[vapply(items, function(x) "Timeframe" %in% names(pkg_data(x)), logical(1))]
  expect_gt(length(items), 40L)

  bad <- .offenders(items, function(d) {
    !is.character(d$Timeframe) ||
      any(is.na(d$Timeframe)) ||
      !all(d$Timeframe %in% c("chronic", "short_term"))
  })
  expect_identical(bad, character(0), label = "datasets with an unexpected Timeframe value")
})

test_that("short_term curated records are confined to marine chlorine", {
  # Timeframe drives the exclusion from allchronic_data (CLAUDE.md section 6).
  # The only short-term curated data are the ANZG (29) and CSIRO (30) marine
  # chlorine records; a new short_term record appearing anywhere else changes
  # what the chronic pipeline drops and must be a deliberate decision.
  items <- shipped_data_frames()
  items <- items[!grepl("_data$", items)]
  items <- items[vapply(items, function(x) "Timeframe" %in% names(pkg_data(x)), logical(1))]

  short_term <- .offenders(items, function(d) any(d$Timeframe == "short_term"))
  expect_setequal(short_term, c("anzg_chlorine_marine", "csiro_chlorine_marine"))

  expect_true(all(pkg_data("anzg_chlorine_marine")$Timeframe == "short_term"))
  expect_true(all(pkg_data("csiro_chlorine_marine")$Timeframe == "short_term"))
})
