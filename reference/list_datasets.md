# List EnviroTox Dataset Names (Deprecated)

This function was named `list_datasets()` in the standalone `envirotox`
package. It has been renamed to
[`envirotox_data_sets()`](https://open-aims.github.io/ssddata/reference/envirotox_data_sets.md)
in `ssddata`. Please update your scripts to use
[`envirotox_data_sets()`](https://open-aims.github.io/ssddata/reference/envirotox_data_sets.md)
instead.

## Usage

``` r
list_datasets()
```

## Value

A character vector of envirotox dataset names.

## See also

[`envirotox_data_sets()`](https://open-aims.github.io/ssddata/reference/envirotox_data_sets.md)

## Examples

``` r
# Deprecated - use envirotox_data_sets() instead. Warns, then returns the
# same value as envirotox_data_sets().
list_datasets()
#> Warning: `list_datasets()` was renamed to `envirotox_data_sets()` in ssddata. Please update your code to use `envirotox_data_sets()` instead.
#> [1] "envirotox_acute"    "envirotox_chemical" "envirotox_chronic" 
#> [4] "envirotox_data"    
```
