...
#   cross-source dedup key in `scripts/stage4c-dedup.R` (J-DEVIATION,
...
# Same explicit col_types as scripts/stage4c-dedup.R -- readr's default
...
if (!wqbench_already_coded) {
  ...
  if (length(anztox_outside_idx) == 0) {
    ...
    # envirotox classifier in scripts/stage4b-extract.R (normalise_effect_text
  ...
  if (nrow(mor_should_be_mort) == 0 && nrow(mort_should_be_mor) == 0) {
    ...
    # via scripts/stage4b-extract.R. Flagging this rather than silently
    ...
    message(
      ...
      "scripts/stage4b-extract.R's envirotox section (or a targeted fixup ",
