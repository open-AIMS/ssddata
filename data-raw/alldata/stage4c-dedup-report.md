...
**J-DEVIATION resolved (Stage 4c Part 3):** in the prior run of this report, `effect_category` was removed from the cross-source key by explicit user decision taken mid-session, because it was not yet a shared vocabulary across sources -- wqbench retained its own literal English-word vocabulary (`effect_category = effect`), while anztox and envirotox used MORT/GRO/REP-style codes, and including it produced *zero* cross-source candidate groups anywhere in the file. `scripts/stage4c-effect-category-fixup.R` has since harmonised wqbench's English words and anztox's free-text tail onto the shared codes (unmappable values set to NA rather than guessed), so `effect_category` is restored to the key for this run:
