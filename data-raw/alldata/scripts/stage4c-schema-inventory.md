...
`data-raw/alldata/scripts/stage4a-supplementary-db-audit.R`:

- `data-raw/alldata/scripts/stage4c-anztox-db-inventory.R` — live DB queries (Windows
  Positron only, requires `infogathering` PostgreSQL connection)
- `data-raw/alldata/scripts/stage4c-wqbench-inventory.R` — loads
  ...
  touched)
- `data-raw/alldata/scripts/stage4c-envirotox-inventory.R` — loads
...
`data-raw/alldata/scripts/stage4c-anztox-db-inventory.R`; this report instead gives the
...
- A ~24-row controlled abbreviation subset, each with a genuine
  ...
  vocabulary already documented in `data-raw/alldata/scripts/stage4a-pipeline-audit.md`:
...
in `data-raw/alldata/scripts/stage4a-pipeline-audit.md`) — they are present at the
