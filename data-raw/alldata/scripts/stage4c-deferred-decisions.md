...
The Phase 1 within-source duplicate diagnostic (`data-raw/alldata/scripts/stage4c-dedup.R`)
...
and/or `medium` per `data-raw/alldata/scripts/stage4c-schema-inventory.md`) match another row
...
`data-raw/alldata/scripts/stage4b-extract.R`: `source_id` was constructed as
...
fixed** in this session: `data-raw/alldata/scripts/stage4b-extract.R` now assigns
`source_id = as.character(row_number())` for wqbench, matching the other two
sources' convention. `data-raw/alldata/scripts/stage4b-extract.R` and
`data-raw/alldata/scripts/stage4b-effect-category-fixup.R` were both re-run end-to-end; all
...
24-column RDS inventory in `data-raw/alldata/scripts/stage4c-schema-inventory.md` Step 2a)
...
then an update to `data-raw/alldata/scripts/stage4c-dedup.R` (currently halts cleanly after
...
**Outcome:** `data-raw/alldata/scripts/stage4c-dedup.R` Phase 1 now passes for all three
